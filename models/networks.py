...
class GraphNet_0(torch.nn.Module):
    def __init__(self, args):
        ...
        self.node_conv = GATConv(self.num_features, self.nhid // self.heads, heads=self.heads, edge_dim=self.edge_dim)
    
    def forward(self, data):
        x, edge_index, edge_attr, batch = data #*.x, data.edge_index, data.batch
        edge_attr[torch.isnan(edge_attr)] = 0
        x = act_fn[self.hidden_act](self.node_conv(x, edge_index, edge_attr=edge_attr))
...
class GraphNet_2(torch.nn.Module):
    ...
    def forward(self, data):
        x, edge_index, edge_attr, batch = data #*.x, data.edge_index, data.batch
        x_node = act_fn[self.hidden_act](self.node_conv(x, edge_index, edge_attr=edge_attr))
...
class GraphNet_3(torch.nn.Module):
    def __init__(self, args):
        ...
        self.node_conv = GATConv(self.num_features, self.nhid // self.heads , heads=self.heads, edge_dim=self.edge_dim) #* // self.heads
