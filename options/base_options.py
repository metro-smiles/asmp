...
class BaseOptions():
	...
	def initialize(self):
		self.parser.add_argument('--data_path', default='/your_data_root/Audiocaps/solo/', help='path to frame/audio/detections')
		...
		self.parser.add_argument('--name', type=str, default='audiocaps_noclassif_graphcomb_noaug', help='name of the experiment. It decides where to store models')
		...
		#audio arguments
		self.parser.add_argument('--audio_window', default=65535, type=int, help='audio segment length') #* 110250
	...
	def parse(self):
		...
		# save to the disk
		self.opt.name = self.opt.name + '_graph_net_' + str(self.opt.gnet_type) + '_graph_lr_' + str(self.opt.lr_graph_net) + '_graph_wt_' + \
		 str(self.opt.graph_loss_weight) + '_batchSize_' + str(self.opt.batchSize)
